...
def deploy_functions(args):
    ...
    if (
        ...
        if serverless_provider == "ow":
            ...
            if function_path is None or function_path == "":
                print(
                    "No function(s) for test T"
                    + test_number
                    + " of provider "
                    + serverless_provider
                )
            ...
            function_url = get_ow_function_url(
            ...
            serverless_deploy_response = deploy(function_path, serverless_provider)
            function_url = get_aws_function_url(
                serverless_deploy_response,
            ...
            serverless_deploy_response = deploy(function_path, serverless_provider)
            function_url = get_azure_function_url(
                serverless_deploy_response,
                get_azure_function_name_and_package(test_number),
                test_number,
            )
            ...
            serverless_deploy_response = deploy(function_path, serverless_provider)
            function_url = get_google_function_url(
                serverless_deploy_response,
        ...
        if serverless_provider == "ow":
            ...
            for prog_lang, path in function_path.items():
                ...
                if path == None or path == "":
                    ...
                    function_url = get_ow_function_url(
                        serverless_deploy_response,
                        get_ow_function_language_name_and_package(
                            test_number, prog_lang
                        ),
                    )

                    if function_url is not None:
                        save_function_language_url(
                            serverless_provider, test_number, function_url, prog_lang
                        )
                        print(
                            "Function in "
                            + prog_lang
                            + " for specified test is deployed with success on OW!\n"
            ...
            for prog_lang, path in function_path.items():
                ...
                if path == None or path == "":
                    ...
                    function_url = get_aws_function_url(
                        serverless_deploy_response,
                        get_aws_function_language_name_and_package(
                            test_number, prog_lang
                        ),
                    )
                    ...
                    if function_url is not None:
                        ...
                        print(
                            "Function in "
                            + prog_lang
                            + " for specified test is deployed with success on AWS!\n"
                            "The url of function is: " + function_url
                        )
                    else:
                        print(
        ...
        if serverless_provider == "ow":
            ...
            for prog_memory, path in function_path.items():
                ...
                if path == None or path == "":
                    ...
                    function_url = get_ow_function_url(
                        serverless_deploy_response,
                        get_ow_function_language_name_and_package(
                            test_number, prog_memory
                        ),
                    )

                    if function_url is not None:
                        save_function_language_url(
                            serverless_provider, test_number, function_url, prog_memory
                        )
                        print(
                            "Function with "
                            + prog_memory
                            + " MB of memory for specified test is deployed with success "
                            "on OW!\nThe url of function is: " + function_url
                        )
                    else:
                        print(
            ...
            for prog_memory, path in function_path.items():
                ...
                if path == None or path == "":
                    ...
                    function_url = get_google_function_url(
                        serverless_deploy_response,
                        get_google_function_language_name_and_package(
                            test_number, prog_memory
                        ),
                    )
...
def remove_functions(args):
    ...
    test_number = args[1]
    print(
        "Removing function(s) for test T"
        + test_number
        + " of provider "
        + serverless_provider
    )
    ...
    if test_number == "5" or test_number == "6":
        ...
        for prog_lang, path in function_path.items():
            if function_path == None or function_path == "":
                ...
                save_function_language_url(
                    serverless_provider, test_number, "", prog_lang
                )
...
def remove(function_path):
    aux = subprocess.check_output(
        ["serverless", "remove"], cwd=str(os.getcwd()) + "/" + str(function_path)
    )
    return aux
...
def get_ow_function_name_and_package(test_number):
    ...
    pack_name = (
        config["owFunctions"]["T" + test_number]["service"]
        + "/"
        + config["owFunctions"]["T" + test_number]["function"]
    )
...
def get_ow_function_language_name_and_package(test_number, prog_lang):
    ...
    pack_name = (
        config["owFunctions"]["T" + test_number][prog_lang]["service"]
        + "/"
        + config["owFunctions"]["T" + test_number][prog_lang]["function"]
    )
...
def get_google_function_language_name_and_package(test_number, prog_lang):
    ...
    pack_name = (
        "/" + config["googleFunctions"]["T" + test_number][prog_lang]["function"]
    )
...
def get_azure_function_name_and_package(test_number):
    ...
    pack_name = (
        config["azureFunctions"]["T" + test_number]["service"]
        + "/"
